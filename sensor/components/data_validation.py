...
class DataValidation:
    ...
    def initiate_data_validation(self)-> DataValidationArtifact:
        try:
            ...
            if not status:
                error_message = f'{error_message}Test data set not contained all columns.\n'
            # Validate numerical columns
            status = self.is_numerical_column_exist(dataframe=train_dataframe)
            if not status:
                error_message = f'{error_message}Train datafrome does not contain all numerical columns.\n'
            
            status = self.is_numerical_column_exist(dataframe=test_dataframe)
            if not status:
                error_message = f'{error_message}Test datafrome does not contain all numerical columns.\n'
